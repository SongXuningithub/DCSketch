...
MultiResBitmapSS::MultiResBitmapSS(){
    V.resize(c);
...
uint32_t MultiResBitmapSS::get_ones_num(uint32_t layer){
...
void MultiResBitmapSS::update(uint32_t l, uint32_t setbit){
    if(l < c - 1){
...
uint32_t MultiResBitmapSS::get_cardinality(){
    int base = c - 2;
    while(base >= 0){
        uint32_t setmax;
        if(base == c - 1)
...
void SpreadSketch::update(string flowid, string elementid){
    ...
    uint32_t* hashres32_arr = (uint32_t*)hashres128.cbegin();
    uint32_t l = get_leading_zeros(hashres32);
    uint32_t setbit;
    if(l < MultiResBitmapSS::c - 1){
        setbit = hashres32 % MultiResBitmapSS::b;
    } else {
        setbit = hashres32 % MultiResBitmapSS::b_hat;
    }
    
    for(size_t i = 0;i < r;i++){
        // uint32_t idx = hashres32_arr[i] % w; //str_hash32(flowid + to_string(i), HASH_SEED_2) % w;
        uint32_t idx = *(hashres32_arr + i) % w;
