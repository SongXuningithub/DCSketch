...
void write_perflow_spread(string dataset,string filename,DCSketch& dcsketch)
{
    ...
    if(per_src_flow)
    {
        ...
        ofile_hand = ofstream(ofile_path + filename.substr(filename.size() - 4) + "offline.txt");
    }
    else
    {
    ...
    while(!ifile_hand.eof())
    {
        ...
        ifile_hand >> flowid;
        ifile_hand >> spread;
        if(flowid == "163189054223")
        {
            cout<<flowid<<endl;
        }
        uint32_t estimated_spread_offline = dcsketch.query_spread_offline(flowid);
        //uint32_t estimated_spread = dcsketch.query_spread(flowid);
         
        ofile_hand << flowid <<" "<<spread<<" "<<estimated_spread_offline<<endl;
        pro_num++;
        if(pro_num%1000 == 0)
            cout<<pro_num<<endl;
    }
